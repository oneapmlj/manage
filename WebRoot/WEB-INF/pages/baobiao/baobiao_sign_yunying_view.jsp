...
<body>
	...
	<div class="public_main width_3_quarter_customer">
		<div class="header">
			<h3 class="tabs_involved">${liucunView.inTimeStart}-${liucunView.inTimeEnd}注册(${liucunView.count})
			<s:if test="%{liucunView.type==2}">到${liucunView.dataTimeEnd}下载(${liucunView.length}/${liucunView.percent}%)
			</s:if>
			<s:if test="%{liucunView.type==3}">到${liucunView.dataTimeEnd}应用(${liucunView.length}/${liucunView.percent}%)
			</s:if>
			</h3>
		</div> 
