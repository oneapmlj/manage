...
public class SignDaoImpl extends DaoImplBase<Long> {
        protected static final Logger LOG = LoggerFactory.getLogger(SignDaoImpl.class);
        protected final String TABLE_NAME = "sign";

        static {
                Instance = new SignDaoImpl();
        }

        private final static SignDaoImpl Instance;
        ...
        public List<Long> findByTime(String start, String end) {
                try {
                        ...
                        while (cursor.hasNext()) {
                                ids.add(Long.parseLong(cursor.next().get("user_id").toString().trim()));
                        }
                        return ids;
                } catch (Exception e) {
                        LOG.error(e.getMessage(), e);
        ...
        public boolean findByIdAndTime(Long userId) {
                try {
                        DBObject object = new BasicDBObject();
                        // object.put("data_time", new BasicDBObject("$gte",
                        // dataTime));
